...
module arp_engine (
    ...
    output logic [47:0] resolved_addr,
    output logic [47:0] reply_target,
    output logic resolved_addr_valid,
    output logic replying
);
...
logic need_to_reply, replied_tick, replied_tick_n;
...
always_ff @(posedge clk) begin
    if(reset) begin
        ...
        timer <= timer_n;
        replied_tick <= replied_tick_n;
    end
end

assign reply_target = reply_target_mac;
always_comb begin
    ...
    case (trans_state)
        ...
        s_send_request: begin
            case(trans_byte)
                ...
                14: source_data = THIS_IP[31:24];
                15: source_data = THIS_IP[23:16];
                16: source_data = THIS_IP[15:8];
                17: source_data = THIS_IP[7:0];
                // Target HW address field ignored - 18, 19, 20, 21, 22, 23
                24: source_data = OTHER_IP[31:24];
                25: source_data = OTHER_IP[23:16];
                26: source_data = OTHER_IP[15:8];
                27: begin   
                    source_data = OTHER_IP[7:0];
                    source_last = 1;
                end
            endcase
            source_valid = 1;
        ...
        s_send_reply: begin
            ...
            case(trans_byte)
                ...
                24: source_data = reply_target_ip[31:24];
                25: source_data = reply_target_ip[23:16];
                26: source_data = reply_target_ip[15:8];
                27: begin   
                    source_data = reply_target_ip[7:0];
                    source_last = 1;
                end
            endcase
            source_valid = 1;
        ...
        s_idle: begin
            ...
            if(need_to_reply && ~replied_tick) begin
                trans_state_n = s_send_reply;
                trans_byte_n = 0;
            end
        end
...
//Reception
logic [4:0] recv_byte;
logic trash_packet, payload_finished;
logic [15:0] latched_op;
logic [47:0] addr_temp; // Staging area for address - potentially invalid

assign sink_ready = ~need_to_reply;
            ...
            if(sink_ready && sink_valid) begin
                ...
                if(~trash_packet) begin
                    ...
                    case (recv_byte)
                        ...
                        // Sender IP address
                        14: if(latched_op == OP_REPLY) expected_byte = OTHER_IP[31:24]; else check_byte = 0;
                        15: if(latched_op == OP_REPLY) expected_byte = OTHER_IP[23:16]; else check_byte = 0;
                        16: if(latched_op == OP_REPLY) expected_byte = OTHER_IP[15:8]; else check_byte = 0;
                        17: if(latched_op == OP_REPLY) expected_byte = OTHER_IP[7:0]; else check_byte = 0;

                        // Target eth address
                        18: if(latched_op == OP_REPLY) expected_byte = THIS_ETH_ADDR[47:40]; else check_byte = 0;
                        19: if(latched_op == OP_REPLY) expected_byte = THIS_ETH_ADDR[39:32]; else check_byte = 0;
                        20: if(latched_op == OP_REPLY) expected_byte = THIS_ETH_ADDR[31:24]; else check_byte = 0;
                        21: if(latched_op == OP_REPLY) expected_byte = THIS_ETH_ADDR[23:16]; else check_byte = 0;
                        22: if(latched_op == OP_REPLY) expected_byte = THIS_ETH_ADDR[15:8]; else check_byte = 0;
                        23: if(latched_op == OP_REPLY) expected_byte = THIS_ETH_ADDR[7:0]; else check_byte = 0;

                        // Target IP address
                        24: expected_byte = THIS_IP[31:24];
                        25: expected_byte = THIS_IP[23:16];
                        26: expected_byte = THIS_IP[15:8];
                        27: if(~payload_finished) expected_byte = THIS_IP[7:0]; else check_byte = 0;
                    endcase
                    ...
                    if(check_byte && expected_byte != sink_data) begin
                        trash_packet <= ~sink_last;
                        recv_byte <= 0;
                        payload_finished <= 0;
                    end else if(recv_byte == ARP_LEN-1) begin
                        payload_finished <= 1;
                        if(sink_last) begin
                            recv_byte <= 0;
                            payload_finished <= 0;
                            ...
                            if(latched_op == OP_REPLY) begin
                                resolved_addr <= addr_temp;
                                resolved_addr_valid <= 1;
                            end else if(latched_op == OP_REQUEST) begin
                                need_to_reply <= 1;
                            end
                        end
                    end else if(sink_last) begin
                        trash_packet <= 0;
                        recv_byte <= 0;
                        payload_finished <= 0;
                    end else
